...
import { BasePlugin, Errors, Events, Sniffer, Util } from 'xgplayer'
import { MSE } from 'xgplayer-streaming-shared'
import { ERROR_CODES, ERROR_TYPES } from './error'
...
export default class Mp4Plugin extends BasePlugin {
   ...
   */
  _startDegradedPlayback(_err, preState) {
  ...
  _onTimeUpdate() {
    ...
    if (mse && mp4 && mp4.canDownload) {
      ...
      const cacheMaxTime = player.paused
        ? player.currentTime + config.minBufferLength
        : player.currentTime + config.maxBufferLength
      if (range[1] - cacheMaxTime < 0) {
        timeRange.every((item, idx) => {
          ...
          if (
            !this._isChangeDefinition &&
            item.endTime - item.startTime > 1 &&
            this._isInBuffer(item)
          ) {
            item.downloaded = true
            item.isLoading = true
            this.log(
              `onTimeUpdate, ${idx} download segment, has buffer`,
              item.startTime,
              item.endTime
            )
            return true
          ...
          if (item.startTime - player.currentTime < config.maxBufferLength) {
            ...
            this.log(
              ...
              player.currentTime,
              ', bufferLen,',
              range[1] - player.currentTime,
              ',bufferRangeList,',
              this.player.buffered2 ? this.player.buffered2.bufferedList : null
            )
            this._loadData()
          }
        })
  ...
  checkRemoveOldBitrateBuffer() {
    ...
    if (
      mse &&
      this._removeBuffeEndTime > 0 &&
      player.currentTime > this._removeBuffeEndTime + 1
    ) {
  ...
  _loadDataSuccess = data => {
    ...
    this.log(
      '[loadFragment] _loadDataSuccess ',
      JSON.stringify(data.context.range),
      ',dataLen,',
    ...
    try {
      ...
      if (
        ...
        context.fragIndex === this.mp4.timeRange.length - 1
      ) {
        ...
        this.log(
          'loaded ended !!!==>>>',
          JSON.stringify(context.range),
          ', fragIndex,',
          context.fragIndex,
          ', bufferEndTime,',
          this.bufferEndTime,
          ',meta_duration,',
          this.mp4.meta.duration
        )
      }
      if (
        this.mse &&
    ...
    if (buffer.end > 0 && buffer.end - player.currentTime >= 2) {
      if (this._waitAdjustTimeCnt < config.waitJampBufferMaxCnt) {
        this._waitInBufferTimer = setTimeout(() => {
          ...
          this.log(
            '[waitInBufferTimeout], waitAdjustTimeCnt,',
            this._waitAdjustTimeCnt,
            ',curtime,',
            curTime,
            util.nowTime()
  ...
  _onSeeking = async () => {
    ...
    if (buffered.end > 0) {
      ...
      if (mp4.meta.duration - buffered.end < 0.5) {
        this._startProgress()
        this.log(
          '[seeking in buffered range], buffer end,',
          buffered.end,
          ', duration,',
          mp4.meta.duration
        )
      ...
      if (this._curLoadSegmentIdx === fragIndex) {
        this._startProgress()
        this.log(
          '[seeking in buffered range], seek fragIndex is current load segmentIdx',
          fragIndex
        )
        return
      }
      ...
      this.log(
        '[seeking in buffered range], seekTime ',
        curTime,
        ',bufferRange,',
        buffered.start,
        '-',
  ...
  _checkRemoveSourceBuffer(sourceBufferRange, currentTime, mustClear) {
    ...
    if (
      (!mustClear &&
        util.nowTime() - this._checkRemoveBufferLastTime <=
          this.config.removeBufferLen) ||
      this.endofstream
    )
      return
    ...
    if (
      sourceBufferRange &&
      sourceBufferRange[0] >= 0 &&
      (currentTime - sourceBufferRange[0] > this.config.removeBufferLen || mse.isFull())
    ) {
      ...
      if (segmentIdx >= 0 && mp4.timeRange[segmentIdx].startTime < currentTime) {
        const clearEnd = Math.floor(
          Math.min(mp4.timeRange[segmentIdx].startTime, sourceBufferRange[1])
        )
        if (sourceBufferRange[0] < clearEnd) {
          this.log(
            '[checkremoveSourceBuffer], remove range==>>>',
            sourceBufferRange[0],
            clearEnd
          )
  ...
  _isEnded() {
    ...
    if (!this.endofstream && this.mse && mp4.meta.duration - player.currentTime < 0.5) {
      this.log(
        '[check player isEnded],deal mse.endOfStream, currentTime,',
        player.currentTime,
        ', bufferend,',
        _end,
        ', duration,',
    ...
    if (mp4?.meta && mp4.meta.duration - player.currentTime < 0.5) {
      this._stopProgress()
      this.log(
        '[check player isEnded],stopProgress and endOfStream,currentTime, ',
        player.currentTime,
        ', bufferend,',
        _end,
        ', duration,',
  ...
  oldChangeDefinition(to, from) {
    ...
    this.log(
      '[oldChangeDefinition],currentTime,',
      player.currentTime,
      ',from,',
      from,
      ',to,',
  ...
  changeDefineCanPlay = (currentTime, paused, from, to) => {
    ...
    player.currentTime = this._changeDefState
      ? this._changeDefState.currentTime
      : currentTime
  ...
  changeDefinition = async (to, from) => {
    ...
    fragIndex < 0 && (fragIndex = this._curLoadSegmentIdx)
    this.log(
      'switchBitrate:point,fragIndex,',
      fragIndex,
      ',startTime,',
      mp4.timeRange[fragIndex].startTime,
      ',currentTime,',
      player.currentTime
    )
    if (mp4) {
  ...
  log(message, ...optionalParams) {
    ...
    const newMessage = playerConfig?.vid
      ? `[Index]${playerConfig.vid} ${message}`
      : `[Index] ${message}`
