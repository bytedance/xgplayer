...
import { FMP4Demuxer, FMP4Remuxer, MP4Demuxer } from 'xgplayer-transmuxer'
import { ERROR_CODES, ERROR_TYPES, NetWorkError, ParserError } from './error'
...
class MP4 extends EventEmitter {
  ...
  constructor(url, options) {
    ...
    if (this.enableWorker && typeof Worker !== 'undefined') {
      try {
        ...
        this.transmuxerWorkerControl = new TransmuxerWorkerControl({
          id: this.workerSequence,
        ...
        this.transmuxerWorkerControl.on('transmux', muxRes => {
          ...
          this.log(
            '[transmuxerworker end] ,range, ',
            JSON.stringify(res.range),
            ',dataLen,',
            res.buffer.byteLength,
            res.context
          )
          this._loadSuccessCallBack?.(res)
        })
  ...
  async changeBitRate(bitRateInfo) {
    ...
    await this.MP4Loader.changeUrl(
      this.url,
      this.options.vid + Date.now(),
      this.CHUNK_SIZE
    )
  ...
  log(message, ...optionalParams) {
    const { options } = this
    const newMessage = options?.vid
      ? `[MP4] ${options.vid} ${message}`
      : `[MP4] ${message}`
   ...
   * @param { string } state
   */
  async errorHandler(error, _state, _context = {}) {
  ...
  getTimeRange() {
    ...
    if (this.audioTrak && this.audioTrak.length > range.length) {
      for (let j = range.length; j < this.audioTrak.length; j++) {
        timeRange = {
          startTime: Math.max(
            this.audioTrak[j].startTime,
            timeRange ? timeRange.endTime : 0
          ),
  ...
  async getMetaInfo(isEmitMetaEvent = true) {
    try {
      ...
      const onProgressHandle = async (data, state, options) => {
        this.log(
          'getMetaInfo onProgressHandle, dataLen,',
          data ? data.byteLength : -1,
          ', state,',
          state,
          ',range,',
        ...
        if (options.meta && !this.meta) {
          ...
          this.meta = {
            ...metaInfo.meta,
            ext: { videoTrak: this.videoTrak, audioTrak: this.audioTrak }
          }
      ...
      await this.MP4Loader.loadMetaProcess(
        this.MP4Loader.cache,
        [0, this.CHUNK_SIZE],
        onProgressHandle
      )
    } catch (error) {
  ...
  async _checkHasMeta() {
    ...
    this._metaLoading = true
    const metaInfo = await this.MP4Loader.loadMeta(
      this.MP4Loader.cache,
      Math.round(this.CHUNK_SIZE / 2)
    )
    this._metaLoading = false
    this.videoTrak = metaInfo.videoSegments
    this.audioTrak = metaInfo.audioSegments
  ...
  getFragRange(fragIndex) {
    ...
    if (this.videoTrak) {
      videoSeg =
        fragIndex < this.videoTrak.length
          ? this.videoTrak[fragIndex]
          : this.videoTrak[this.videoTrak.length - 1]
    }
    let audioSeg = null
    if (this.audioTrak) {
      audioSeg =
        fragIndex < this.audioTrak.length
          ? this.audioTrak[fragIndex]
          : this.audioTrak[this.audioTrak.length - 1]
    }
    let videodiff = 0,
      audiodiff = 0
  ...
  async load(fragIndex, successCallback) {
    ...
    if (this._switchBitRate && !this._metaLoading) {
      this.log(
        '[switchBitrate], switch bitRate start load, time,',
        fragIndex >= this.videoTrak.length
          ? this.audioTrak[fragIndex].startTime
          : this.videoTrak[fragIndex].startTime,
        ', fragIndex,',
        fragIndex
      )
      ...
      if (this.changeBitRateTime > 0) {
        if (
          this.timeRange[fragIndex].startTime <= this.changeBitRateTime &&
          this.changeBitRateTime < this.timeRange[fragIndex].endTime
        ) {
          this.log(
            '[switchBitrate], not need update load fragIndex',
            fragIndex,
            ',stTime,',
            this.changeBitRateTime
          )
        } else {
          // 需要根据新码率的关键帧列表去找切换后码率的开始下载的fragIndex，修复不同码率关键帧不对齐导致卡播
          fragIndex = this.getFragmentIdx(this.changeBitRateTime)
          this.log(
            '[switchBitrate], need update load fragIndex',
            fragIndex,
            ',stTime,',
            this.changeBitRateTime,
            ',newBitrateTimeRange,',
            this.timeRange[fragIndex].startTime,
            '-',
            this.timeRange[fragIndex].endTime
          )
  ...
  getSubRange(fragIndex, time, range) {
    ...
    if (this.videoTrak) {
      ...
      for (let j = 0; j < keyFrameList.length; j++) {
        this.log(
          '>>>>>getSubRange video keyFrameList, startTime,',
          keyFrameList[j].startTime / videoTimescale,
          ',range,',
          keyFrameList[j].offset
        )
      }
      for (; i < keyFrameList.length; i++) {
        ...
        if (
          startTime <= time &&
          time < endTime &&
          range[0] < keyFrameList[i - 1].offset
        ) {
          videoStartRange = keyFrameList[i - 1].offset
          find = true
          this.log(
            '>>>>>getSubRange video end, startTime,',
            startTime,
            ',endTime,',
            endTime,
            ',startRange,',
      ...
      if (!find && startTime <= time && time < videoSeg.endTime + 0.8) {
        videoStartRange = keyFrameList[i - 1].offset
        this.log(
          '>>>>>getSubRange video last, startTime,',
          startTime,
          ',endTime,',
          videoSeg.endTime,
          ',startRange,',
    ...
    if (this.audioTrak) {
      ...
      i = Math.floor(
        (time * audioTimescale - frameList[0].startTime) / audioSeg.frames[0].duration
      )
      i = Math.min(frameList.length - 1, i)
      let starttime =
        i > 0
          ? frameList[i - 1].startTime / audioTimescale
          : frameList[0].startTime / audioTimescale
      for (; i >= 0 && i < frameList.length; ) {
        ...
        if (starttime <= time && time < endtime && range[0] < frameList[i].offset) {
          ...
          find = true
          this.log(
            '>>>>>getSubRange audio end, startTime,',
            starttime,
            ',endTime,',
            endtime,
            ',startRange,',
  ...
  onprogressDataArrive = async (data, state, options) => {
    if (data && data.byteLength > 0) {
      ...
      this.log(
        '[mp4.loadFragment onprogressDataArrive ] receive data, >>> index,',
        options.index,
        ',range,',
        JSON.stringify(options.range),
        ', dataLen,',
  ...
  _mux(buffer, start, fragIndex, state) {
    ...
    if (this.transmuxerWorkerControl && !this.meta.isFragmentMP4) {
      ...
      this.log(
        '[transmuxerworker start] ,range, ',
        JSON.stringify(range),
        ',dataLen,',
        buffer.byteLength,
        context
      )
      this.transmuxerWorkerControl.transmux(
        this.workerSequence,
        buffer,
        start,
        videoIndexRange,
      ...
      try {
        ...
        if (this.meta.isFragmentMP4) {
          ...
          demuxRet = this.MP4Demuxer.demuxPart(
            buffer,
            start,
            videoIndexRange,
            audioIndexRange,
        ...
        if (!this.FMP4Remuxer && (!this.checkCodecH265() || this.options.supportHevc)) {
          this.FMP4Remuxer = new FMP4Remuxer(demuxRet.videoTrack, demuxRet.audioTrack, {
            openLog: checkOpenLog()
          })
        }
        let res
        this.log(
          '[mux], videoTimeRange,',
          demuxRet.videoTrack
            ? [demuxRet.videoTrack.startPts, demuxRet.videoTrack.endPts]
            : null,
          ',audioTimeRange,',
          demuxRet.audioTrack
            ? [demuxRet.audioTrack.startPts, demuxRet.audioTrack.endPts]
            : null
        )
        const startPts = Math.min(
          demuxRet.videoTrack.startPts,
          demuxRet.audioTrack.startPts
        )
        const endPts = Math.max(demuxRet.videoTrack.endPts, demuxRet.audioTrack.endPts)
        const timeRange = [startPts, endPts]
        if (this.FMP4Remuxer) {
          const remuxRes = this.FMP4Remuxer.remux(this._needInitSegment, {
            initMerge: true,
            range: range
          })
        ...
        const err = new ParserError(ERROR_TYPES.remux, ERROR_CODES.muxError, {
  ...
  async loadFragment(fragIndex, range) {
    if (
      this._isPending ||
      (range.length > 0 && range[0] === 0 && range[1] === 0) ||
      this.timeRange[fragIndex].isLoading
    ) {
      return
    }
    this.log(
      '[MP4.loadFragment] ,fragIndex,',
      fragIndex,
      ',range ',
      range,
      ',len ,',
      range[1] - range[0],
      ', bufferLoaded_Len,',
      this.bufferLoaded.byteLength
    )
    if (
      range.length >= 2 &&
      range[1] &&
      range[1] > 0 &&
      range[1] <= this.bufferLoaded.byteLength
    ) {
      ...
      if (range.length >= 2 && range[0] && range[0] <= this.bufferLoaded.byteLength) {
        if (!this.timeRange[fragIndex].isLoading) {
          ...
          if (buffer.byteLength > 0) {
            ...
            this.log(
              '[mp4.loadFragment] has part data: ',
              start,
          ...
          if (!this._metaLoading && !this.timeRange[fragIndex].isLoading) {
            this.log(
              '[mp4.loadFragment] ready to load part data >>> ',
              this.bufferLoaded.byteLength,
              range[1]
            )
            this.timeRange[fragIndex].isLoading = true
        ...
        if (
          (!this._metaLoading || range[0] >= this.CHUNK_SIZE) &&
          !this.timeRange[fragIndex].isLoading
        ) {
          this.timeRange[fragIndex].isLoading = true
          this.log(
            '[mp4.loadFragment],ready to load all data ,segmentIdx, ',
            fragIndex,
            ',range >>> ',
            JSON.stringify(range)
          )
  ...
  checkCodecH265() {
    const isHvc =
      this.meta &&
      (this.meta.videoCodec.indexOf('hvc1') > -1 ||
  ...
  destroy() {
    ...
    this.resetTansmuxer()
    this.transmuxerWorkerControl?.destroy()
  ...
  resetTansmuxer() {
    this.MP4Demuxer?.reset()
    this.MP4Demuxer = null
    this.FMP4Remuxer?.reset()
    this.FMP4Remuxer = null
    this.transmuxerWorkerControl?.reset()
  }
}

export { MP4 as default, MP4_EVENTS }
