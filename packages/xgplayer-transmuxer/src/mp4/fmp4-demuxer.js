...
export class FMP4Demuxer {
  ...
  demuxPart(partData, partDataStart, moov) {
    ...
    if (this.__lastRemainData) {
      const lastRemainDataEnd =
        this.__lastRemainDataStart + this.__lastRemainData.byteLength
      // 如果遗留数据和新数据之间存在非包含关系的重叠，则需要合并，否则丢弃遗留数据
      // 如果遗留数据和新数据之间不存在重叠，需要丢弃遗留数据，保证后续的新数据可以连续的解析
      const continuous =
        partDataStart <= lastRemainDataEnd &&
        partDataStart > this.__lastRemainDataStart &&
        partDataStart + partData.byteLength > lastRemainDataEnd
      if (continuous) {
        // data = 遗留数据 + 新数据，为了程序健壮性，即可能存在重复的range，需要subarray规避
        const noDuplicateData = partData.subarray(
          this.__lastRemainData.byteLength + this.__lastRemainDataStart - partDataStart
        )
    ...
    if (data) {
      ...
      if (this.__nextMoofStart < 0) {
        ...
        this.__nextMoofStart >= dataStart &&
        this.__nextMoofStart <= dataEnd - 8
      ) {
        MP4Parser.findBox(
          data.subarray(this.__nextMoofStart - dataStart),
          ['moof'],
          this.__nextMoofStart
        ).forEach(v => moofBoxes.push(v))
      }
      moofBoxes
        .filter(moofBox => moofBox.size <= moofBox.data.length)
        .forEach(moofBox => {
          ...
          this.__nextMoofStart =
            ...
            Math.max(
              ...moof.traf.map(v =>
                v.trun.samples.reduce((ret, w) => ret + w.size, v.trun.dataOffset || 0)
              )
            )
          ...
          if (videoTrack.id === k) {
            tracks[k].some(x => {
              ...
              const sample = new VideoSample(
                (x.pts || x.dts) + videoBaseMediaDecodeTime,
                x.dts + videoBaseMediaDecodeTime
              )
            ...
            tracks[k].some(x => {
              ...
              audioTrack.samples.push(
                new AudioSample(x.dts + audioBaseMediaDecodeTime, sampleData, x.duration)
              )
              this.__lastRemainDataStart = xStart + x.size
            })
          }
    ...
    if (
      ...
      this.__lastRemainDataStart < data.byteLength + dataStart
    ) {
  ...
  demux(videoData, audioData) {
    ...
    if (audioData) {
      ...
      if (moofBox) {
        const samples = MP4Parser.moofToSamples(
          MP4Parser.moof(moofBox),
          null,
          audioTrack
        )[audioTrack.id]
        ...
        if (samples) {
          ...
          samples.map(x => {
            ...
            audioTrack.samples.push(
              new AudioSample(x.dts + baseMediaDecodeTime, sampleData, x.duration)
            )
          })
        }
      }
    ...
    if (videoData) {
      ...
      if (moofBox) {
        const tracks = MP4Parser.moofToSamples(
          MP4Parser.moof(moofBox),
          videoTrack,
          audioTrack
        )
