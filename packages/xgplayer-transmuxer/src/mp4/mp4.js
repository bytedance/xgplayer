...
export class MP4 {
  ...
  static ftyp(tracks) {
    const isHevc = tracks.find(
      t => t.type === TrackType.VIDEO && t.codecType === VideoCodecType.HEVC
    )
  ...
  static pssh(data) {
    ...
    const content = new Uint8Array(
      [
        ...
        [
          0x10, 0x77, 0xef, 0xec, 0xc0, 0xb2, 0x4d, 0x02, 0xac, 0xe3, 0x3c, 0x1e, 0x52,
          0xe2, 0xfb, 0x4b
        ],
        [0x00, 0x00, 0x00, 0x01],
        parse(data.kid),
  ...
  static moov(tracks) {
    if (tracks[0].useEME && (tracks[0].encv || tracks[0].enca)) {
      ...
      const pssh = MP4.pssh(tracks[0].pssh)
      // console.log('[remux],pssh,', hashVal(pssh.toString()))
      return MP4.box(
        MP4.types.moov,
        MP4.mvhd(
          tracks[0].mvhdDurtion || tracks[0].duration,
          tracks[0].mvhdTimecale || tracks[0].timescale
        ),
        MP4.mvex(tracks),
  ...
  static mvhd(duration, timescale = 90000) {
    const mvhd = MP4.box(
      ...
      new Uint8Array([
        ...
        0xff,
        0xff,
        0xff // todo 0xff, 0xff, 0xff, 0xff // next_track_ID
      ])
    )
  ...
  static tkhd(id, duration, width = 0, height = 0) {
    const tkhd = MP4.box(
      ...
      new Uint8Array([
        ...
        (duration >> 16) & 0xff,
        (duration >> 8) & 0xff,
        duration & 0xff, // todo (duration >> 24) & 0xff, (duration >> 16) & 0xff, (duration >> 8) & 0xff, duration & 0xff,
        0x00,
        0x00,
        0x00,
        0x00,
        ...
        0x00, // layer
        0x00,
        0x00, // /*0x00*/ alternate_group  //todo type === 'video' ? 0x01 : 0x00（第二个位置）
        0x01,
        0x00, // /* 0x01 */, 0x00, // non-audio track volume  //todo type === 'audio' ? 0x01 : 0x00（第一个位置）
        0x00,
        0x00, // reserved
        0x00,
        0x01,
  ...
  static mdia(track) {
    const mdia = MP4.box(
      MP4.types.mdia,
      MP4.mdhd(track.duration, track.timescale),
      MP4.hdlr(track.type),
      MP4.minf(track)
    )
  ...
  static mdhd(duration, timescale = 90000) {
    const mdhd = MP4.box(
      ...
      new Uint8Array([
        ...
        0x00,
        0x00,
        0x00, // todo 0x00, 0x00, 0x00, 0x00, // modification_time
        (timescale >> 24) & 0xff,
        (timescale >> 16) & 0xff,
        (timescale >> 8) & 0xff,
        timescale & 0xff,
        ...
        (duration >> 16) & 0xff,
        (duration >> 8) & 0xff,
        duration & 0xff, // (duration >> 24) & 0xff, (duration >> 16) & 0xff, (duration >> 8) & 0xff, duration & 0xff,//todo
        0x55,
        0xc4, // 'und' language (undetermined) //todo 0x15, 0xC7
        0x00,
        0x00 // pre_defined
      ])
    )
  ...
  static minf(track) {
    const minf = MP4.box(
      MP4.types.minf,
      track.type === TrackType.VIDEO ? MP4.VMHD : MP4.SMHD,
      MP4.DINF,
      MP4.stbl(track)
    )
  ...
  static encv(track) {
    ...
    const avcc = new Uint8Array(
      [
        ...
        .concat(...sps)
        .concat([0x01, (pps.length >>> 8) & 0xff, pps.length & 0xff])
        .concat(...pps)
    )
    const btrt = new Uint8Array([
      0x00, 0x00, 0x58, 0x39, 0x00, 0x0f, 0xc8, 0xc0, 0x00, 0x04, 0x56, 0x48
    ])
    ...
    return MP4.box(
      ...
      MP4.box(MP4.types.btrt, btrt),
      sinf,
      MP4.box(MP4.types.pasp, pasp)
    )
  }
  ...
  static sinf(data) {
    ...
    return MP4.box(
      MP4.types.sinf,
      content,
      MP4.box(MP4.types.frma, frma),
      MP4.box(MP4.types.schm, schm),
      schi
    )
  }
  static av01(track) {
  ...
  static hvcC(track) {
    ...
    if (hvcC) {
      ...
      data = [
        0x01, // configurationVersion
        (hvcC.generalProfileSpace << 6) |
          (hvcC.generalTierFlag << 5) |
          hvcC.generalProfileIdc,
        pcf >>> 24,
        pcf >>> 16,
        pcf >>> 8,
        pcf,
      ...
      data = [
        ...
        0x00,
        0x00,
        0x00, // general_profile_compatibility
        0x90,
        0x00,
        0x00,
        0x00,
        ...
        0x00,
        0xfc,
        0xfd, // profile_compatibility_indications
        ...
        0xa0,
        0x00,
        0x01, // array_completeness + ‘0’b + NAL_unit_type + numNalus
        0x00,
        0x18, // nalUnitLength
        0x40,
        0x01,
  ...
  static moof(tracks) {
    const moof = MP4.box(
      MP4.types.moof,
      MP4.mfhd(tracks[0].samples ? tracks[0].samples[0].gopId : 0),
      ...tracks.map(t => MP4.traf(t))
    )
  ...
  static traf(track) {
    ...
    if (!track.useEME || (!track.isVideoEncryption && !track.isAudioEncryption)) {
      ...
      return MP4.box(
        ...
        tfdt,
        sdtp,
        MP4.trun(track.samples, sdtp.byteLength + offset)
      )
  ...
  static sdtp(data) {
    ...
    return MP4.box(MP4.types.sdtp, MP4.extension(0, 0), buffer.buffer)
  }
  ...
  static senc(data) {
    ...
    buffer.write(
      Buffer.writeUint32(16 + samplesLength),
      MP4.types.senc,
      MP4.extension(0, flag)
    )
  ...
  static sbgp() {
    ...
    return MP4.box(MP4.types.sbgp, MP4.extension(0, 0), content)
  }
  ...
  static trun(samples, offset) {
    ...
    for (let i = 0; i < sampleLen; i++) {
      const { duration, size, flag = {}, cts = 0 } = samples[i]
      data.set(
        [
          (duration >>> 24) & 0xff,
          (duration >>> 16) & 0xff,
          ...
          size & 0xff,
          (flag.isLeading << 2) |
            (flag.dependsOn === null || flag.dependsOn === undefined
              ? 1
              : flag.dependsOn),
          (flag.isDependedOn << 6) |
            (flag.hasRedundancy << 4) |
            (flag.paddingValue << 1) |
            (flag.isNonSyncSample === null || flag.isNonSyncSample === undefined
              ? 1
              : flag.isNonSyncSample),
          flag.degradationPriority & (0xf0 << 8),
          flag.degradationPriority & 0x0f, // sample_flags
          (cts >>> 24) & 0xff,
          (cts >>> 16) & 0xff,
          (cts >>> 8) & 0xff,
    ...
    entries.forEach(({ firstChunk, samplesPerChunk, sampleDescIndex }) => {
      data.set(
        [
          ...
          sampleDescIndex >> 24,
          (sampleDescIndex >> 16) & 0xff,
          (sampleDescIndex >> 8) & 0xff,
          sampleDescIndex & 0xff
        ],
        offset
      )
      offset += 12
  ...
  static stsz(samplesSizes) {
    ...
    samplesSizes.forEach(x => {
      data.set([x >> 24, (x >> 16) & 0xff, (x >> 8) & 0xff, x & 0xff], offset)
      offset += 4
    })
    return MP4.box(
      MP4.types.stsz,
  ...
  static styp() {
    return MP4.box(
      MP4.types.styp,
      new Uint8Array([
        0x6d, 0x73, 0x64, 0x68, 0x00, 0x00, 0x00, 0x00, 0x6d, 0x73, 0x64, 0x68, 0x6d,
        0x73, 0x69, 0x78
      ])
    )
  }
