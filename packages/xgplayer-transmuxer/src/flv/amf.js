...
export class AMF {
  static parse(data) {
    ...
    const ret = {}
    const name = AMF._parseValue(
      new DataView(data.buffer, data.byteOffset, data.byteLength)
    )
    const value = AMF._parseValue(
      new DataView(data.buffer, data.byteOffset + name.size, data.byteLength - name.size)
    )
  ...
  static _parseValue(view) {
    ...
    switch (type) {
      ...
      case 2:
        {
          // String type
          const { data, size } = AMF._parseString(
            new DataView(view.buffer, view.byteOffset + offset, view.byteLength - offset)
          )
          value = data
          offset += size
        }
      ...
      case 3:
        {
          ...
          while (offset < dataLen - 4) {
            ...
            const { size, data, isEnd } = AMF._parseObject(
              new DataView(
                view.buffer,
                view.byteOffset + offset,
                view.byteLength - offset - terminal
              )
            )
            if (isEnd) break
      ...
      case 10:
        {
          ...
          for (let i = 0; i < strictArrayLength; i++) {
            const { data, size } = AMF._parseValue(
              new DataView(
                view.buffer,
                view.byteOffset + offset,
  ...
  static _parseObject(view) {
    ...
    const value = AMF._parseValue(
      new DataView(view.buffer, view.byteOffset + name.size, view.byteLength - name.size)
    )
