import { AAC, AVC, HEVC, MPEG, NALu } from '../codec'
import {
  AudioSample,
  AudioTrack,
  MetadataTrack,
...
import { concatUint8Array, Logger } from '../utils'
...
export class TsDemuxer {
        ...
        case this._pmtId:
          {
            if (payloadUnitStartIndicator) offset += data[offset] + 1
            const tableEnd =
              offset + 3 + (((data[offset + 1] & 0x0f) << 8) | data[offset + 2]) - 4
      ...
      switch (type) {
        ...
        case 40: // HEVC SUFFIX_SEI
          ...
          this.metadataTrack.seiSamples.push(
            new SeiSample(NALu.parseSEI(NALu.removeEPB(unit), isHevc), pts)
          )
  ...
  _parseAacData(pes) {
    ...
    if (pts === null || pts === undefined) {
      ...
      pts =
        track.samples[track.samples.length - 1].pts +
        AAC.getFrameDuration(track.sampleRate)
    }
  ...
  _parseMPEG(pes) {
    ...
    while (offset < length) {
      if (MPEG.isHeader(data, offset)) {
        const frame = MPEG.appendFrame(this.audioTrack, data, offset, pts, frameIndex)
        if (frame) {
          offset += frame.length
          frameIndex++
        } else {
  ...
  static _parsePES(data) {
    ...
    if (
      headerDataLen === null ||
      headerDataLen === undefined ||
      data.length < headerDataLen + 9
    )
      return
