...
export default class Mp4Remuxer {
  ...
  remux() {
    const {
      audioTrack,
      videoTrack
    } = this._context.getInstance('TRACKS');
  ...
  _remuxVideo(videoTrack) {
    ...
    let moofMdat = new Buffer(); // track.samples = mp4Samples
    // track.time = firstDts

    const moof = Fmp4.moof(track);
    const mdat = Fmp4.mdat(mdatBox);
    moofMdat.write(moof, mdat); // this._videoSegmentList.append(videoSegment)
    // if (!this._store.isLive) {
    // }

    track.samples = [];
    track.length = 0;
    this.emit(REMUX_EVENTS.MEDIA_SEGMENT, 'video', moofMdat); // this.handleMediaFragment({
    //   type: 'video',
  ...
  _remuxAudio(track) {
    ...
    const mdat = Fmp4.mdat(mdatBox);
    moofMdat.write(moof, mdat); // if (!this._store.isLive) {
