import Buffer from 'xgplayer-utils/dist/write/Buffer'; // const UINT32_MAX = Math.pow(2, 32) - 1;
...
class Fmp4 {
  ...
  static mvhd(duration, timeScale) {
    let timescale = timeScale || 1000; // duration *= timescale;
  ...
  static edts(data) {
    ...
    buffer.write(Fmp4.size(36), Fmp4.type('edts')); // elst
  ...
  static avc1(data) {
    ...
    let vSpacing = data.parRatio.width; // let avccBuffer = new Buffer()
  ...
  static trun(data, sdtpLength) {
    ...
    // let ceil = id === 1 ? 16 : 12;
    let buffer = new Buffer();
    let sampleCount = Buffer.writeUint32(data.samples.length); // mdat-header 8
    // moof-header 8
    // mfhd 16
    // traf-header 8
    // thhd 16
  ...
  static mdat(data) {
    ...
    data.samples.forEach(item => {
      item.buffer.forEach(unit => {
        ...
        offset += unit.data.byteLength; // buffer.write(unit.data);
      });
    });
    return mdatBox;
  }
