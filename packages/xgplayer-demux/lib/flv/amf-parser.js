...
var AMFParser = function () {
  ...
  _createClass(AMFParser, [{
    ...
    value: function parseString(buffer) {
      ...
      var strLen = dv.getUint16(0, !_xgplayerUtils.isLe);
      var str = '';
      if (strLen > 0) {
    ...
    value: function parseDate(buffer, size) {
      ...
      var ts = dv.getFloat64(0, !_xgplayerUtils.isLe);
      var timeOffset = dv.getInt16(8, !_xgplayerUtils.isLe);
    ...
    value: function parseLongString(buffer) {
      var dv = new DataView(buffer, this.readOffset);
      var strLen = dv.getUint32(0, !_xgplayerUtils.isLe);
      var str = '';
      if (strLen > 0) {
        str = _xgplayerUtils.UTF8.decode(new Uint8Array(buffer, this.readOffset + 2, strLen));
      } else {
    ...
    value: function parseValue(data, size) {
      ...
      switch (type) {
        ...
        case OBJECT:
          {
            ...
            if (offset <= size - 3) {
              var mark = dataView.getUint32(offset - 1, !_xgplayerUtils.isLe) & 0x00FFFFFF;
              if (mark === 9) {
                this.readOffset += 3;
                offset += 3;
              }
        ...
        case MIX_ARRAY:
          {
            ...
            if (offset <= size - 3) {
              var marker = dataView.getUint32(offset - 1, !_xgplayerUtils.isLe) & 0x00FFFFFF;
              if (marker === 9) {
                offset += 3;
                this.readOffset += 3;
              }
        ...
        case STRICT_ARRAY:
          {
            value = [];
            var arrLength = dataView.getUint32(1, !_xgplayerUtils.isLe);
            offset += 4;
            this.readOffset += 4;
