...
require('./polyfills/custom-elements.min');
...
var _workerTicker = require('./helpers/worker-ticker');

var _workerTicker2 = _interopRequireDefault(_workerTicker);
...
var MobileVideo = function (_HTMLElement) {
  ...
  function MobileVideo() {
    ...
    _this._fps = _ticker.DEFAULT_FPS;
    return _this;
  }
  ...
  _createClass(MobileVideo, [{
    ...
    value: function init() {
      ...
      this.vCtx.on(_videoContext.VIDEO_CANVAS_EVENTS.VIDEO_EVENTS, this.handleVCtxInnerEvent);
    }
  }, {
    key: '_initAudio',
    value: function _initAudio() {
      ...
      this.ticker = new _workerTicker2.default({ interval: 1000 / this._fps });
      // this.ticker = new (getTicker({interval: 1000 / this._fps}))()
      this.reconciler = new _reconciler2.default({
        vCtx: this.vCtx,
        aCtx: this.aCtx,
        video: this
      });
    ...
    set: function set(val) {
      if (!(0, _ticker.validateFPS)(val)) {
        val = _ticker.DEFAULT_FPS;
      }
      this._fps = val;
      if (this.ticker) {
