var Render = (function () {
  ...
  function (_Filter) {
    ...
    function Yuyv422(render, config) {
      var _this;
    ...
    _createClass(Render, [{
      ...
      value: function _drawPicture(data, width, height) {
        ...
        for (var i = 0; i < this.filters.length - 1; i++) {
          // TODO
          var filter = this.filters[i];
          var _inputTexture = tempTexture;
          tempTexture = GLUtil.createTexture(gl, gl.LINEAR, new Uint8Array(width * height * 4), width, height);
          gl.bindFramebuffer(gl.FRAMEBUFFER, this.fb);
          gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, tempTexture, 0);
          filter.render(_inputTexture, width, height);
        }
