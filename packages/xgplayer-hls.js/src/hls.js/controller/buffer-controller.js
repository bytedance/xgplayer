  ...
  onSBUpdateEnd () {
    ...
    if(this._paused === false
      ...
      && timeRanges["audio"].end(0) - timeRanges["audio"].start(0) > 1) {
        ...
        let startTime = Math.max(timeRanges["video"].start(0),timeRanges["audio"].start(0))
    ...
    for (let trackName in tracks) {
      if (!sourceBuffer[trackName]) {
        ...
        try {
          ...
          if(err.code == 22) {
            logger.log("Found new track, try to rebuild");
            this._paused = false;
            // setup the media source
            let ms = this.mediaSource = new MediaSource();
            // Media Source listeners
            ...
            for(let i=0;i<Object.keys(this.tracks).length;i++) {
              let track = this.tracks[Object.keys(this.tracks)[i]];
              this.pendingTracks[Object.keys(this.tracks)[i]] = track;
            }
            this.tracks = {};
            this.sourceBuffer = {};
            ms.addEventListener('sourceopen', this.onmso);
