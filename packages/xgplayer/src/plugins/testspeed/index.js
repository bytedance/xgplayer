...
export default class TestSpeed extends BasePlugin {
  ...
  testSpeed = () => {
    ...
    const testSpeedUrl =
      url + (url.indexOf('?') < 0 ? '?testst=' : '&testst=') + Date.now()
    if (this.cnt >= testCnt) {
      return
    }
    this.cnt++
  ...
  appendList = (speed, type = DEFAULT_SPEED_TYPE) => {
    ...
    if (player) {
      type === DEFAULT_SPEED_TYPE
        ...
        : (player[this.getSpeedName('realTime', type)] = speed)
    }
    ...
    if (player) {
      if (type === DEFAULT_SPEED_TYPE) {
        if (!player.avgSpeed || speed !== player.avgSpeed) {
          ...
          player.emit(Events.DOWNLOAD_SPEED_CHANGE, {
            speed: speed,
  ...
  destroy() {
    ...
    SPEED_TYPE.forEach(type => {
      this.speedListCache?.[type] && (this.speedListCache[type] = [])
    })
