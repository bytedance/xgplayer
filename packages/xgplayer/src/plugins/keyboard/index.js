...
class Keyboard extends BasePlugin {
        ...
        if (!this.keyCodeMap[key]) {
          ...
          ;[
            ...
            extendkeyCodeMap[key][key1] &&
              (this.keyCodeMap[key][key1] = extendkeyCodeMap[key][key1])
          })
        }
      })
    }
  ...
  checkIsVisible() {
    ...
    if (
      (top < 0 && top < 0 - height * 0.9) ||
      (bottom > 0 && bottom - h > height * 0.9)
    ) {
      return false
    }
    return true
  ...
  checkCode(code, isBodyTarget) {
    ...
    Object.keys(this.keyCodeMap).map(key => {
      if (
        this.keyCodeMap[key] &&
        code === this.keyCodeMap[key].keyCode &&
        !this.keyCodeMap[key].disable
      ) {
  ...
  seek(event) {
    const { currentTime, offsetCurrentTime, duration, offsetDuration, timeSegments } =
      this.player
    ...
    if (!event.repeat && !_keyState.isKeyDown) {
      if (
        (e.target === document.body ||
          (this.config.isGlobalTrigger && !isDisableTag(e.target))) &&
        this.checkCode(keyCode, true)
      ) {
  ...
  onKeydown = event => {
    ...
    if (!e.repeat) {
      if (
        this.config.disable ||
        this.config.disableRootTrigger ||
        e.metaKey ||
        e.altKey ||
        e.ctrlKey
      ) {
      ...
      if (
        e &&
        (e.keyCode === 37 || this.checkCode(e.keyCode)) &&
        (e.target === this.player.root ||
          e.target === this.player.video ||
          e.target === this.player.controls.el)
      ) {
  ...
  handleKeyCode(curKeyCode, event, isPress) {
    ...
    for (let i = 0; i < arr.length; i++) {
      const { action, keyCode, disable, pressAction, disablePress } =
