...
class AudioCtx extends _events2.default {
  constructor(config) {
    ...
    this._volume = this.config.volume || 0.6; // 记录外部传输的状态
  ...
  decodeAAC() {
    ...
    try {
      this.context.decodeAudioData(buffer.buffer, function (buffer) {
        ...
        audioSource.buffer = buffer; // audioSource.onended = _this.onSourceEnded.bind(_this);
  ...
  static getAdts(meta, data) {
    let adts = new Uint8Array(7); // 设置同步位 0xfff 12bit

    adts[0] = 0xff;
    adts[1] = 0xf0; // Object data (没什么人用MPEG-2了，HLS和FLV也全是MPEG-4，这里直接0)  1bit
    // Level always 00 2bit
    // CRC always 1 1bit

    adts[1] = adts[1] | 0x01; // profile 2bit

    adts[2] = 0xc0 & meta.objectType - 1 << 6; // sampleFrequencyIndex

    adts[2] = adts[2] | 0x3c & meta.sampleRateIndex << 2; // private bit 0 1bit
    // chanel configuration 3bit

    adts[2] = adts[2] | 0x01 & meta.channelCount >> 2;
    adts[3] = 0xc0 & meta.channelCount << 6; // original_copy: 0 1bit
    // home: 0 1bit
    ...
    adts[5] = 0xe0 & aacframelength << 5; // adts_buffer_fullness 0x7ff 11bit

    adts[5] = adts[5] | 0x1f;
    adts[6] = 0xfc; // number_of_raw_data_blocks_in_frame 0 2bit;
