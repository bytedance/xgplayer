...
class MPD extends EventEmitter {
        ...
        .then(res => {
          ...
          if (ctx) {
            ...
            if (result.mediaPresentationDuration) {
              this.duration = util.durationConvert(result.mediaPresentationDuration)
            }
            let MpdBaseURL = ''
            if (result.BaseURL) {
            ...
            if (!this.duration && Period && Period.duration) {
              this.duration = util.durationConvert(Period.duration)
            }
            meta.duration = this.duration
            Period.AdaptationSet.forEach((asItem, _asIndex) => {
              ...
              asItem.Representation.forEach((rItem, _rIndex) => {
                ...
                if (ST) {
                  ...
                  const end = start + Math.ceil(this.duration / segmentDuration) - 1
                  for (let i = start; i <= end; i++) {
                    const startTime = segmentDuration * (i - start)
                    let endTime = segmentDuration * (i - start + 1)
                    if (i === end) {
                      segmentDuration = this.duration - segmentDuration * (end - start)
                    ...
                    mediaSegments.push({
                      ...
                      url:
                        baseURL +
                        ST.media
                          .replace('$RepresentationID$', rItem.id)
                          .replace('$Number%04d$', util.preFixInterge(i, 4)),
                      downloaded: false,
                      segmentDuration
                    })
                  }
                ...
                if (mimeType === 'video/mp4') {
                  this.mediaList.video.push({
                    ...
                    baseURL,
                    initSegment:
                      baseURL + initSegment.replace('$RepresentationID$', rItem.id),
                    inited: false,
                    mediaSegments,
                    mimeType,
                    codecs,
            ...
            ;['video', 'audio'].forEach(mediaType => {
              this.mediaList[mediaType].selectedIdx = 0
              this.mediaList[mediaType].sort((a, b) => {
  ...
  init(url) {
    this.fetch(url).then(
      res => {
        this.type = res.meta.TYPE.toLocaleLowerCase()
        this.emit('ready')
      },
      () => {
        console.log('mpd init reject')
        ;['video', 'audio'].forEach(mediaType => {
          this.mediaList[mediaType] = []
        })
        this.retry++
  ...
  seek(time) {
    ...
    if (this.type === 'vod' || time !== undefined) {
      ;['video', 'audio'].forEach(mediaType => {
        const mo = this.mediaList[mediaType][this.mediaList[mediaType].selectedIdx]
        if (mo) {
          mediaResults[mediaType] = mo.mediaSegments.filter(item => {
            return (
              ((item.start <= time && time < item.end) ||
                (item.start <= time - item.segmentDuration &&
                  time - item.segmentDuration < item.end) ||
                (item.start <= time + item.segmentDuration &&
                  time + item.segmentDuration < item.end)) &&
              !item.downloaded
            )
