...
class Lyric {
  constructor(txts, dom) {
    this.rawTxts = txts
    this.txts = txts.map(item => {
      return item.replace(/^[\r\n]|[\r\n]$/g, '').match(/(\[.*\])[^[]+/g)
    })
    this.isDynamics = txts.map((item, idx) => {
      return (
        [].concat(item.match(/\[\d{2}:\d{2}\.\d{2,3}\]/g)).length ===
          this.txts[idx].length &&
        this.txts[idx].length === this.txts[0].length &&
        this.txts[idx].length > 1
      )
    })
    ...
    this.list = this.lists.reduce((pre, cur) => {
      return pre.map((item, idx) => {
        return {
          ...
          lyric:
            item.lyric === '\n'
              ? `${item.lyric}${cur[idx].lyric}`
              : `${item.lyric}<br/>${cur[idx].lyric}`,
          idx
        }
      })
    })
  ...
  find(curTime) {
    ...
    return list.filter(({ time }, idx) => {
      ...
      return (
        curTime >= time &&
        ((list[idxy] && curTime * 1 + interval * 1 <= list[idxy].time) ||
          idxy >= list.length)
      )
    })
  }
  ...
  bind(player) {
    ...
    if (this.isDynamic) {
      ...
      player.on('timeupdate', this.__handle__)

      this.__startHandle__ = (() => {
        player.emit('lyricUpdate', this.list[0])
      }).bind(this, player)
      player.once('playing', this.__startHandle__)
      //
      // self.__endHandle__ = (() => {
  ...
  show() {
    ...
    if (dom && dom.nodeType === 1) {
      ...
      this.list.forEach(item => {
        lyrbicTxts.push(
          `<xg-lyric-item class="xgplayer-lyric-item" data-idx="${item.idx}">${item.lyric.replace(/[\r\n]/g, '')}</xg-lyric-item>`
        )
      })
      lrcWrap.innerHTML = lyrbicTxts.join('')
      const lrcForward = Util.createDom(
        'xg-lrcForward',
        '<div></div>',
        {},
        'xgplayer-lrcForward'
      ...
      ev.forEach(item => {
        lrcForward.addEventListener(
          item,
          e => {
            e.preventDefault()
            e.stopPropagation()
            this.offset -= this.offsetScale
            console.log(`lyric go forward ${this.offsetScale}s`)
          },
          false
        )
      })
      ...
      ev.forEach(item => {
        lrcBack.addEventListener(
          item,
          e => {
            e.preventDefault()
            e.stopPropagation()
            this.offset += this.offsetScale
            console.log(`lyric go back ${this.offsetScale}s`)
          },
          false
        )
      })
      this.__updateHandle__ = item => {
        ...
        if (activeDom) {
          ...
          activeTop =
            ...
            domWrap.getBoundingClientRect().top +
            domWrap.scrollTop -
            offsetHeight / 2
          if (activeTop) {
